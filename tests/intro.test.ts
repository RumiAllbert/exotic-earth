...
test('intro respects daily limit, reduced motion, anchors, storage failure and skip', () => {
  const now = 200000000;
  for (const scenario of ['first', 'recent', 'expired', 'reduced', 'anchor', 'storage']) {
    const events: Record<string, () => void> = {};
    const intro = { hidden: true, addEventListener: (key: string, fn: () => void) => { events[key] = fn; } };
    let saved = '';
    runInNewContext(script, {
      document: { getElementById: () => intro }, location: { hash: scenario === 'anchor' ? '#about' : '' },
      matchMedia: () => ({ matches: scenario === 'reduced' }), Date: { now: () => now },
      localStorage: { getItem: () => { if (scenario === 'storage') throw Error(); return scenario === 'recent' ? String(now - 1000) : scenario === 'expired' ? '1' : null; }, setItem: (_: string, value: string) => { saved = value; } },
    ...
    const show = scenario === 'first' || scenario === 'expired';
    assert.equal(intro.hidden, !show, scenario);
    if (show) { assert.equal(saved, String(now)); events[scenario === 'first' ? 'wheel' : 'timeout'](); assert.equal(intro.hidden, true); assert.equal(events.keydown, undefined); }
  }
});
