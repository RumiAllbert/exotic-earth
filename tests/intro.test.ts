...
test('intro respects daily limit, reduced motion, anchors, storage failure and skip', () => {
  const now = 200000000;
  for (const scenario of ['first', 'recent', 'expired', 'reduced', 'anchor', 'storage', 'local']) {
    const events: Record<string, () => void> = {};
    const intro = { hidden: true, addEventListener: (key: string, fn: () => void) => { events[key] = fn; } };
    let saved = '';
    runInNewContext(script, {
      document: { getElementById: () => intro }, location: { hostname: scenario === 'local' ? '127.0.0.1' : 'rumicalles.com', hash: scenario === 'anchor' ? '#about' : '' },
    ...
    const show = scenario === 'first' || scenario === 'expired' || scenario === 'local';
    assert.equal(intro.hidden, !show, scenario);
    if (show) { assert.equal(saved, scenario === 'local' ? '' : String(now)); events[scenario === 'first' ? 'wheel' : 'timeout'](); assert.equal(intro.hidden, true); assert.equal(events.keydown, undefined); }
  }
});
