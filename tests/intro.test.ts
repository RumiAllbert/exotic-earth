...
test('intro respects daily limit, reduced motion, anchors, storage failure and skip', () => {
  const now = 200000000;
  for (const scenario of ['first', 'recent', 'expired', 'reduced', 'anchor', 'storage', 'local', 'local-anchor']) {
    ...
    const show = scenario === 'first' || scenario === 'expired' || scenario.startsWith('local');
    assert.equal(intro.hidden, !show, scenario);
    assert.equal(events.scroll, undefined);
    if (show) { assert.equal(saved, scenario.startsWith('local') ? '' : String(now)); events[scenario === 'first' ? 'wheel' : 'timeout'](); assert.equal(intro.hidden, true); assert.equal(events.keydown, undefined); }
  }
});
